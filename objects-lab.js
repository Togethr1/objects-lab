...
// // console.dir == essentially displays every item, rather thant the first 100 items in an array 
...
console.log("Your starter Pokemon is:", starterPokemon.name);

// ORGINAL ^ just selects first starter, what if it was random? 

// const randomPokemon = pokemon.filter(p => p.starter); 
// const ranPoke = Math.floor(Math.random() * randomPokemon.length);
// const starterPokemon = randomPokemon[ranPoke];
// game.party.push(starterPokemon);
// console.log("Your starter Pokemon is:", starterPokemon.name); 

// come back to this --- need to build a loop into exercise 5 to ensure that the starter doesn't get added multiple times
...
const otherPokemon = [
    pokemon.find(p => p.number === 103), 
    pokemon.find(p => p.number === 35),
    pokemon.find(p => p.number === 59),
]

game.party.push(...otherPokemon); 
console.log("Your Party Now Inlcudes:", game.party.map(p => p.name).join(', ')); 
...
console.log("Completed Gyms:", game.gyms.filter(g => g.completed).map(g => g.location).join(','));
...
const evolvedStarterPokemon = game.party.find(p => p.starter === true); 
const evolvedPokemon = pokemon.find(p => p.number === evolvedStarterPokemon.number + 1);
const starterEvolved = game.party.indexOf(evolvedStarterPokemon);
game.party.splice(starterEvolved, 1, evolvedPokemon); 

console.log("Your Evolved Starter Pokemon is:", evolvedPokemon.name);

// game.party.splice(0,1, evolvedPokemon[i]); // this is another way to do it, but not sure if it works with the starter Pokemon being in a different index position
...
game.party.forEach(p => console.log("Your Party Pokemon:", p.name, p.type, p.hp)); 
...
// need a filter method to find all the starter Pokemon


const starterPokemonList = pokemon.filter(p => p.starter); 
starterPokemonList.forEach(p => console.log("starter Pokemon:", p.name));
...
game.catchPokemon = function(pokeObj) {
    ...
    console.log("You caught a new Pokemon:", pokeObj.name);
}

game.catchPokemon(pokemon.find(p => p.number === 150)); 
console.log("You now have:", game.party.map(pokeObj => pokeObj.name).join(', ')); 
...
game.catchPokemon = function(pokeObj) {
    ...
    if (pokeBalls.quantity <= 0) {
        ...
        console.log('You have', pokeBalls.quantity, 'pokeballs left'); 
    }
    }; 
...
//  revise this --- this is throwing me off: (change the value of `complete` in the qualifying objects from false to true). not sure if this is completed or not, need to play with the return value 
...
const tally = gymStatus();
console.log("Gym Tally:", tally);
...
// console.log(game.party); 
// its loging 6 instead of 5 - will debug after completing exercise 15/16
